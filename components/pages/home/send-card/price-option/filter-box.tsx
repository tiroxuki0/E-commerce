...
const Box: React.FC<Props> = ({ onClose }) => {
  ...
  const handleFilter = (item: ItemType) => {
    ...
    router.push({
      pathname: "/",
      query: { ...router.query, price: item.value }
    })
  }

  return (
    <ul>
      {PRICE_OPTIONS.map((item: ItemType, index: number) => {
        return (
          <li className="cursor-pointer" key={index} onClick={() => handleFilter(item)}>
            <a className="flex items-center p-[10px] hover:bg-item">
              <span className={`text-[14px] ${filter === undefined ? (item.value === PRICE_OPTIONS[0].value ? "font-[500]" : "font-normal") : filter === item.value ? "font-bold" : "font-normal"}`}>
                {item.label}
              </span>
            </a>
          </li>
